...
def load_monthly_mean(no2_agg: list, no2_obs_counts: list):
    ...
    # check more-than-one keys
    for k in no2_monthly:
        if len(list(no2_monthly[k].keys())) > 1:
            no2_monthly[k] = no2_monthly[k][[q for q in no2_monthly[k].keys() if str(q) in k.split("_")[1]][0]]
        else:
            no2_monthly[k] = no2_monthly[k][list(no2_monthly[k].keys())[0]]
    ...
    # check more-than-one keys
    for k in no2_monthly_counts:
        if len(list(no2_monthly_counts[k].keys())) > 1:
            no2_monthly_counts[k] = no2_monthly_counts[k][[q for q in no2_monthly_counts[k].keys() if str(q) in k.split("_")[1]][0]]
        else:
            no2_monthly_counts[k] = no2_monthly_counts[k][list(no2_monthly_counts[k].keys())[0]]
...
def get_monthly_weekday_weekend_mean(no2_wkday: dict, no2_wkday_counts: dict):
    ...
    for k in no2_wkday.keys():
        ...
        wkday_agg = wkday_weighted_mean_numerator / wkday_weighted_mean_denominator
